...
namespace processor
{
	infra::Processor::Info Ease_out::get_processor_info()
	{
		return infra::Processor::Info{
			.identifier = "Ease_out",
			.display_name = "Ease out",
			.singleton = false,
			.generate = std::make_unique<Ease_out>
		};
	}

	std::vector<infra::Processor::Pin_attribute> Ease_out::get_pin_attributes() const
	{
		...
		for (int ch = 0; ch < channel_count; ch++)
		{
			...
			if (initial)
				for (int i = 0; i < element_count; i++)
				{
					...
					if (start_volume > 0.0f) start_volume -= volume_rate;
					start_volume = std::max(start_volume, 0.0f);
				}
		}
	};

	void Ease_out::process_payload(
	...
	)
	{
		...
		if (!input_item_optional.has_value())
			throw Runtime_error(
				"Ease out processor has no input",
				"Ease out processor requires an audio stream input to function properly.",
		...
		while (!stop_token)
		{
			...
			if (temp_time > 0) temp_time -= (double)out_frame->nb_samples / out_frame->sample_rate;
			if (temp_time <= 0 && !initial) initial = std::make_unique<bool>();
	...
	void Ease_out::draw_title()
	{
		imgui_utility::shadowed_text("Esae out");
	}

	bool Ease_out::draw_content(bool readonly)
	{
		ImGui::BeginGroup();
		ImGui::BeginDisabled(readonly);
		{
			...
			ImGui::SetNextItemWidth(200);
			ImGui::DragFloat("Ease Out Time", &this->time, 0.1f, 0.0f, 1000.0f, "%.2f");
		}
		ImGui::EndDisabled();
		ImGui::EndGroup();
	...
	Json::Value Ease_out::serialize() const
	{
		Json::Value value;
	...
	void Ease_out::deserialize(const Json::Value& value)
	{
