	...
	namespace
	{
		...
		void DrawNode(ImNodesEditorContext& editor, const int node_idx)
		{
			...
			if (node_selected)
			{
				...
				node_outline = node.ColorStyle.TitlebarSelected;
			}
			else if (node_hovered)
			{
				node_background = node.ColorStyle.BackgroundHovered;
				node_outline = node.ColorStyle.TitlebarHovered;
			}
		...
		ImGui::BeginGroup();
		{
...

			{
				...
				GImNodes->CanvasRectScreenSpace = ImRect(
					EditorSpaceToScreenSpace(ImVec2(0.f, 0.f)),
