...
static inline float ImLog(int x)
{
	return ImLog(static_cast<float>(x));
}
...
namespace ImGuiKnobs
{
	namespace detail
	{
			...
			)
			{
				...
				bool drag_vertical
				   ...
				   && (flags & ImGuiKnobFlags_DragVertical
					   || ImAbs(io.MouseDelta[ImGuiAxis_Y]) > ImAbs(io.MouseDelta[ImGuiAxis_X]));
		...
		)
		{
			...
			ImGui::PushID(label);
			auto width
				= size == 0 ? ImGui::GetTextLineHeight() * 4.0f : size * ImGui::GetIO().FontGlobalScale;
